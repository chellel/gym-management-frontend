<template>
  ...
  >
    <div v-if="coach" class="space-y-6">
      ...
      <div class="bg-white rounded-lg border p-6">
        <div class="flex items-start space-x-6">
          ...
          <div class="flex-shrink-0">
            <el-avatar :size="100" :src="coach.avatar" :alt="coach.name">
              {{ coach.name.charAt(0) }}
            </el-avatar>
          </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div class="space-y-2">
                <div class="flex items-center">
                  <el-icon class="w-5 h-5 text-gray-400 mr-2"><Message /></el-icon>
                  <span class="text-gray-600">{{ coach.email }}</span>
                </div>
                <div class="flex items-center">
                  <el-icon class="w-5 h-5 text-gray-400 mr-2"><Phone /></el-icon>
                  <span class="text-gray-600">{{ coach.phone }}</span>
                </div>
                <div class="flex items-center">
                  <el-icon class="w-5 h-5 text-gray-400 mr-2"><Calendar /></el-icon>
                  <span class="text-gray-600">入职时间：{{ formatDate(coach.hire_date) }}</span>
                </div>
              </div>
              <div class="space-y-2">
                <div class="flex items-center">
                  <el-icon class="w-5 h-5 text-gray-400 mr-2"><Star /></el-icon>
                  <span class="text-gray-600">经验：{{ coach.experience }}</span>
                </div>
              </div>
            </div>
          </div>
      ...
      <!-- 专业信息 -->
      <div class="bg-white rounded-lg border p-6">
        <h3 class="text-lg font-semibold text-gray-900 mb-4">专业信息</h3>
        <div class="space-y-4">
          <div v-if="coach.bio">
      ...
      <div class="bg-white rounded-lg border p-6">
        ...
        <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
          <div class="text-center">
            <div class="text-2xl font-bold text-blue-600">{{ stats.total_classes }}</div>
          ...
          <div class="text-center">
            <div class="text-2xl font-bold text-yellow-600">{{ stats.monthly_classes }}</div>
      ...
      <div class="bg-white rounded-lg border p-6">
        ...
        <div v-if="recentClasses.length > 0" class="space-y-3">
          ...
          >
            <div class="flex-1">
              <div class="font-medium text-gray-900">{{ classItem.activity }}</div>
              <div class="text-sm text-gray-500">
                {{ formatDate(classItem.date) }} {{ classItem.start_time }} - {{ classItem.end_time }}
              </div>
            </div>
...
import { coachApi, generateMockCoachStats, type Coach, type CoachStats } from '@/api/coach'
...
const loadCoachData = async (coachId: number) => {
  try {
    ...
    // 实际API调用
    // const [statsRes, classesRes, reviewsRes] = await Promise.all([
    //   coachApi.getCoachStats(coachId),
    //   coachApi.getCoachSchedule(coachId, { start: getDateRange().start, end: getDateRange().end }),
    //   coachApi.getCoachReviews(coachId)
    // ])
