<template>
  <Layout>
    <div class="space-y-6">
      ...
      <!-- 教练排班视图 -->
      <div v-if="currentView === 'trainers'" class="bg-white shadow overflow-hidden sm:rounded-lg">
          ...
          <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
              <tr>
                ...
                >
                  <div>{{ day.name }}</div>
                  <div class="text-gray-400 font-normal">{{ formatDate(day.date) }}</div>
                </th>
              </tr>
            </thead>
      ...
      <!-- 场地预约视图 -->
      <div v-if="currentView === 'locations'" class="bg-white shadow overflow-hidden sm:rounded-lg">
        <div class="px-6 py-4 border-b border-gray-200">
          ...
          <div class="p-5">
            <div class="flex items-center">
              ...
              <div class="ml-5 w-0 flex-1">
                <dl>
                  <dt class="text-sm font-medium text-gray-500 truncate">总教练数</dt>
                  <dd class="text-lg font-medium text-gray-900">{{ trainers.length }}</dd>
                </dl>
              </div>
            </div>
          </div>
        ...
        <div class="bg-white overflow-hidden shadow rounded-lg">
          <div class="p-5">
            <div class="flex items-center">
              ...
              <div class="ml-5 w-0 flex-1">
                <dl>
                  <dt class="text-sm font-medium text-gray-500 truncate">总场地数</dt>
        ...
        <div class="bg-white overflow-hidden shadow rounded-lg">
          <div class="p-5">
            <div class="flex items-center">
              ...
              <div class="ml-5 w-0 flex-1">
                <dl>
                  <dt class="text-sm font-medium text-gray-500 truncate">本周排班</dt>
                  <dd class="text-lg font-medium text-gray-900">{{ weekSchedules.length }}</dd>
                </dl>
              </div>
            </div>
          </div>
    ...
    <!-- 添加/编辑排班模态框 -->
    <div v-if="showAddScheduleModal || showEditScheduleModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
      <div class="relative top-20 mx-auto p-5 border w-11/12 md:w-3/4 lg:w-1/2 shadow-lg rounded-md bg-white">
        <div class="mt-3">
          ...
          <form @submit.prevent="showAddScheduleModal ? handleAddSchedule() : handleUpdateSchedule()" class="space-y-4">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label for="trainer" class="block text-sm font-medium text-gray-700">教练 *</label>
                <select
                  id="trainer"
              ...
              <div>
                <label for="location" class="block text-sm font-medium text-gray-700">场地 *</label>
                <select
                  id="location"
              ...
              <div>
                <label for="schedule-date" class="block text-sm font-medium text-gray-700">日期 *</label>
                <input
                  id="schedule-date"
              ...
              <div>
                <label for="activity" class="block text-sm font-medium text-gray-700">活动内容 *</label>
                <input
                  id="activity"
              ...
              <div>
                <label for="schedule-start-time" class="block text-sm font-medium text-gray-700">开始时间 *</label>
                <input
              ...
              <div>
                <label for="schedule-end-time" class="block text-sm font-medium text-gray-700">结束时间 *</label>
                <input
...
const getTrainerSchedules = (trainerId, date) => {
  return schedules.value.filter(schedule => 
    schedule.trainer_id === trainerId && schedule.date === date
  )
}
...
const handleAddSchedule = async () => {
  try {
    const newSchedule = {
      id: Date.now(),
      ...scheduleForm,
      trainer_name: trainers.value.find(t => t.id == scheduleForm.trainer_id)?.name,
      location: locations.value.find(l => l.id == scheduleForm.location_id)?.name
    }
