...
- **文件**: `src/views/MemberManagement.vue`
- **功能**:
  - 会员信息 CRUD 操作
  - 多字段搜索（姓名、会员ID、电话）
  - 会员状态筛选（活跃、过期）
...
- **文件**: `src/views/ScheduleManagement.vue`
- **功能**:
  - 教练排班管理
  - 场地预约管理
  - 周视图展示
...
│   ├── Login.vue         # 登录页
│   ├── Welcome.vue       # 欢迎页
│   ├── MemberManagement.vue # 会员管理页
│   ├── ClassBooking.vue     # 课程预约页
│   ├── ScheduleManagement.vue # 排班管理页
│   └── Reports.vue          # 数据报表页
├── composables/          # Vue 3 Composables
│   ├── useAuth.js        # 认证 Composable
│   └── useMembers.js     # 会员数据 Composable
├── services/             # 数据服务 (API 封装)
│   ├── memberService.js  # 会员数据服务
│   └── classService.js   # 课程数据服务
├── router/               # 路由配置
│   └── index.js          # 路由入口
├── App.vue              # 主应用组件
├── main.js              # 应用入口
